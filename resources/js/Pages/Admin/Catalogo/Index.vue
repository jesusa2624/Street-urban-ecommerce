<template>
  <AdminLayout>
      ...
      <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
        ...
        <div v-if="productosFiltrados.length > 0" class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="p in productosFiltrados" :key="p.id" class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td class="py-3 px-4 text-center text-gray-600">{{ p.variantes }}</td>
                ...
                <td class="py-3 px-4 text-right text-gray-900">S/ {{ p.precioVenta.toFixed(2) }}</td>
                <td class="py-3 px-4">
    ...
    <RegistrarModeloModal
      ...
      :marcas-por-categoria="marcasPorCategoria"
      :editando="editando"
      @close="cerrarModal"
    />
  </AdminLayout>
...
const abrirEditar = (producto) => {
  editando.value = { id: producto.id, nombre: producto.nombre, marca: producto.marca, categoria: producto.categoria };
  showModal.value = true;
};
